...
./blender-4.5.3-linux-x64/blender -b v2.0.0.blend --quiet --python gen.py -- --gpu 0 --n_scene 5000 --cycles-device OPTIX
```
Here `--n_scene` is the number of scenes to generate. Adjust these numbers as needed. The `--gpu` flag specifies gpu index (useful for sending jobs). Each scene means randomly spawning blocks, lights and background HDRI.

By default resolution is set to 1280 x 704.
...
sbatch gen.sh
``` 
See `gen.sh` for more details. By default script generate 5000 scenes. Adjust as needed. You can generate on multiple GPUs by submitting multiple jobs with different `--gpu` index. Simply `sbatch gen.sh` multiple times but change the `--gpu` flag.
